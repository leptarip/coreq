...
#
# Adapted to Cython by 278097159+leptarip@users.noreply.github.com (2025)
...
import carla
from source.simulation_environment.cython.controller_cy import VehiclePIDController
from source.simulation_environment.cython.misc_cy import draw_waypoints, get_speed
...
class LocalPlanner(object):
    ...
    def _compute_next_waypoints(self, k=1):
        ...
        available_entries: int = self._waypoints_queue.maxlen - len(self._waypoints_queue)
        k: int = min(available_entries, k)
    ...
    def set_global_plan(self, current_plan, stop_waypoint_creation=True, clean_queue=True):
        ...
        new_plan_length: int = len(current_plan) + len(self._waypoints_queue)
