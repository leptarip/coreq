...
from source.simulation_environment.agents.dynamic_agent import DynamicAgent
...
from source.simulation_environment.scenario.scenario1.scenario1_cfg_actors import EgoActor, AdversaryActor, DeliveryVanActor1, DeliveryVanActor2
from source.simulation_environment.scenario.scenario1.scenario1_cfg_actors import get_position_x_y
...
SCENARIO_1_NAME = "intersection1"

class ScenarioIntersection1(BaseScenario):
    """
    A 4 way intersection. CAV (ego) goes straight.
    ADV turns left on oncoming traffic
    crossing ego's trajectory.
    """
    TAG = f"Scenario:{SCENARIO_1_NAME}"
    ...
    def __init__(self,
        ...
        self.van_actor1 = DeliveryVanActor1(self.world, self.carla_map)
        self.van_actor2 = DeliveryVanActor2(self.world, self.carla_map)
        self.scenario_traj_manager = ScenarioTrajMng()
        self.ego_agent = None
        self.adv_agent = None
        self.van_agent1 = None
        self.van_agent2 = None
        self.obps_manager = None
        self.communication = None
        self.sampler = sampler
        self.seed = seed
        self.list_buildings = None
        self.printl = source.simulation_environment.logs.printl.PrintL(ScenarioIntersection1.TAG, enabled=True)
        self.log_time = []
        self.obs_time = []
    ...
    def _create_all_obps(self, cfg):
        ...
        try:
            sensors = cfg_parser.get_sensors_cfg(cfg, get_position_x_y)
    ...
    def generate_scenario(self) -> BaseScenario.ErrorCode:
        ...
        self.van1 = self.world.spawn_actor(self.van_actor1.get_actor_bp(),
                                           self.van_actor1.get_spawn_transform())
        self.van2 = self.world.spawn_actor(self.van_actor2.get_actor_bp(),
                                           self.van_actor2.get_spawn_transform())
    ...
    def pre_run(self) -> dict:
        ...
        # Built here rather than at spawn time, for the same reason the ego and
        # adversary agents are: BasicAgent's PID controller seeds past_steering from
        # vehicle.get_control(), which returns uninitialised data on an actor the
        # world has never ticked. The settling loop above guarantees a valid read.
        self.van_agent1 = DynamicAgent(self.van1)
        self.van_agent2 = DynamicAgent(self.van2)
        ...
        self.van_agent1.set_global_plan(self.van_actor1.get_global_plan())
        self.van_agent2.set_global_plan(self.van_actor2.get_global_plan())
    ...
    def is_scenario_done(self, sim_time) -> BaseScenario.State:
        ...
        if ego_done is True:
            ...
            log_data = SimScenarioItem(SCENARIO_1_NAME, sim_time, "agent_done", self.delta_time)
        ...
        if ego_route_poly is not None and adv_route_poly is not None:
            ...
            if rs is True:
                key = rd.get_h_key(self.experiment_name, rd.K_SCENARIO_TERMINATION)
                stream_data_name = rd.get_stream_name(self.experiment_name, key.decode())
                log_data = SimScenarioItem(SCENARIO_1_NAME, sim_time, "col_shapes", self.delta_time)
                rd.save_stream_scenario_termination(stream_data_name, log_data)
    ...
    def run_step(self, sim_time, extra) -> BaseScenario.State:
        ...
        adv_route = self.adv_agent.get_route_polygon()
        target = traj_shape.Target(self.adv_agent.get_actor(), adv_route)
        ...
        self.van_agent1.step(sim_time)
        self.van_agent2.step(sim_time)
    ...
    def destroy(self):
        self.destroy_actors(self.ego, self.adv, self.van1, self.van2)
