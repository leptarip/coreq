...
"""
Offline training for the heteroscedastic Gaussian GP surrogate.

Training function accepts train/calib index arrays. It fits the GP on the train
split, calibrates global bias/variance scale on the calib split, and persists
the artifacts.
"""
...
from source.design_exploration.surrogates.hetGP.heteroscedastic_gp import (
    HeteroGPCfg,
...
def train_gp_offline(
    ...
    *,
    ...
    gp_cfg = HeteroGPCfg(seed=seed, d_safe=d_safe)
    ...
    # Build margins map over train+calib to reuse for calibration.
    ...
    mu_raw, std_epistemic_raw, var_aleatoric_raw, _ = predict_violation_probability(
        mean_model, var_model, grid, norm_params=(dataset_train.norm_lo, dataset_train.norm_span)
    )
    ...
    if len(calib_idx) >= 10 and calib_eps_total >= 200:
        bias, var_scale, calib_used_designs, calib_used_eps, cov_lo, cov_mid, cov_hi, calib_used_idx = compute_gaussian_calibration(
            mu_raw,
            var_total_raw,
            dataset_all.margins,
            calib_idx,
        ...
        logger.warning("[gp_offline][calib] insufficient calibration data (designs=%d eps=%d); using bias=0 var_scale=1", len(calib_idx), calib_eps_total)
    ...
    def _compute_logloss(idx_arr: np.ndarray, name: str):
        ...
        for gi in idx_arr:
            ...
            p_unsafe = float(stats_norm.cdf(0.0, loc=mu_adj, scale=np.sqrt(max(var_adj, 1e-12))))
        ...
        if labels and preds:
            ...
            if len(uniq) >= 2:
                ...
                logger.info(
                    "[gp_offline] WARNING: %s set is single-class labels=%s; AUC=N/A",
                    name.upper(),
                    uniq.tolist(),
                )
            logger.info(
                "[gp_offline] %s logloss=%.4f acc=%.4f auc=%s on %d designs (%d episodes)",
                name,
                val,
                acc,
    ...
    out_dir = prepare_run_dir(surrogate_root, "hetgp")
    ...
    logger.info("[gp_offline] saved model=%s meta=%s", model_path, meta_path)
    return {"model": model_path, "meta": meta_path, "bias": bias, "var_scale": var_scale}
